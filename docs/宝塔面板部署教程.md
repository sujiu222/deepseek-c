# 宝塔面板 + Docker 部署教程

## 📋 准备工作

### 1. 服务器要求
- 操作系统：CentOS 7+、Ubuntu 18.04+、Debian 9+
- 内存：≥ 2GB
- CPU：≥ 1核
- 硬盘：≥ 20GB

### 2. 安装宝塔面板

```bash
# CentOS
yum install -y wget && wget -O install.sh https://download.bt.cn/install/install_6.0.sh && sh install.sh

# Ubuntu/Debian
wget -O install.sh https://download.bt.cn/install/install-ubuntu_6.0.sh && sudo bash install.sh
```

安装完成后记录：
- 面板地址：`http://your-ip:8888`
- 用户名
- 密码

---

## 🔧 第一步：安装必要软件

### 1. 登录宝塔面板
访问 `http://your-ip:8888`，使用安装时的用户名密码登录。

### 2. 安装软件（在"软件商店"中）

#### 必装：
- ✅ **Docker** - 容器管理
- ✅ **PostgreSQL 15** - 数据库

#### 可选：
- ✅ **Nginx** - 反向代理（通常已安装）
- ✅ **Docker Compose** - 容器编排

---

## 🗄️ 第二步：配置数据库

### 1. 创建数据库

**宝塔面板 → 数据库 → PostgreSQL → 添加数据库**

- 数据库名：`deepseek-c`
- 用户名：`deepseek_user`
- 密码：`生成强密码`（记录下来）

### 2. 或者使用终端创建

```bash
# 进入 PostgreSQL
sudo -u postgres psql

# 执行 SQL
CREATE DATABASE "deepseek-c";
CREATE USER deepseek_user WITH PASSWORD 'your_strong_password';
GRANT ALL PRIVILEGES ON DATABASE "deepseek-c" TO deepseek_user;
\q
```

---

## 📦 第三步：部署应用

### 1. 创建项目目录

**宝塔面板 → 文件 → /www/wwwroot → 新建目录**
- 目录名：`deepseek-c`

或使用终端：
```bash
cd /www/wwwroot
mkdir deepseek-c
cd deepseek-c
```

### 2. 上传代码

#### 方式 A：使用 Git（推荐）

```bash
cd /www/wwwroot/deepseek-c
git clone https://github.com/sujiu222/deepseek-c.git .
```

#### 方式 B：手动上传
在宝塔面板 → 文件 → 上传文件，将项目压缩包上传并解压。

### 3. 创建生产环境配置

```bash
cd /www/wwwroot/deepseek-c
cp .env.example .env.production
nano .env.production
```

编辑 `.env.production`：

```bash
# DeepSeek API 配置
DEEPSEEK_RPC_URL=https://api.chatanywhere.tech
DEEPSEEK_API_KEY=sk-your-production-api-key

# 数据库配置
# 注意：Docker 容器访问宿主机使用 host.docker.internal
DATABASE_URL=postgresql://deepseek_user:your_strong_password@host.docker.internal:5432/deepseek-c

# 认证配置
NEXTAUTH_URL=https://your-domain.com
NEXTAUTH_SECRET=$(openssl rand -base64 32)

NODE_ENV=production
```

保存并退出（Ctrl + X，然后 Y，然后 Enter）

### 4. 构建 Docker 镜像

```bash
cd /www/wwwroot/deepseek-c
docker build -t deepseek-c .
```

**等待 1-3 分钟**，看到 `Successfully built` 和 `Successfully tagged deepseek-c:latest` 表示成功。

### 5. 启动容器

#### 使用 docker-compose（推荐）

```bash
docker-compose up -d
```

#### 或使用 docker run

```bash
docker run -d \
  --name deepseek-c \
  -p 3000:3000 \
  --env-file .env.production \
  --add-host host.docker.internal:host-gateway \
  --restart unless-stopped \
  deepseek-c
```

### 6. 运行数据库迁移

```bash
docker exec -it deepseek-c npx prisma migrate deploy
```

### 7. 验证运行状态

```bash
# 查看容器状态
docker ps

# 查看日志
docker logs -f deepseek-c
```

看到 `ready started server on 0.0.0.0:3000` 表示启动成功！

---

## 🌐 第四步：配置 Nginx 反向代理

### 方式 1：宝塔面板图形化配置（推荐新手）

#### 1. 添加网站

**宝塔面板 → 网站 → 添加站点**

- 域名：`your-domain.com`
- 备注：`deepseek-c`
- 根目录：`/www/wwwroot/deepseek-c`（随意，不会用到）
- PHP版本：`纯静态`

点击"提交"

#### 2. 配置反向代理

**网站设置（点击域名后的"设置"）→ 反向代理 → 添加反向代理**

- 代理名称：`deepseek-c`
- 目标URL：`http://127.0.0.1:3000`
- 发送域名：`$host`
- 内容替换：留空

点击"提交"

#### 3. 申请 SSL 证书

**网站设置 → SSL → Let's Encrypt**

- 域名：勾选你的域名
- 验证方式：文件验证
- 勾选"强制HTTPS"

点击"申请"，等待几秒钟即可。

### 方式 2：手动编辑配置文件（推荐高级用户）

**网站设置 → 配置文件**

复制以下内容：

```nginx
server {
    listen 80;
    server_name your-domain.com;
    return 301 https://$server_name$request_uri;
}

server {
    listen 443 ssl http2;
    server_name your-domain.com;
    
    # SSL 证书路径（Let's Encrypt 自动生成）
    ssl_certificate /www/server/panel/vhost/cert/your-domain.com/fullchain.pem;
    ssl_certificate_key /www/server/panel/vhost/cert/your-domain.com/privkey.pem;
    
    # SSL 配置
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers ECDHE-RSA-AES128-GCM-SHA256:HIGH:!aNULL:!MD5:!RC4:!DHE;
    ssl_prefer_server_ciphers on;
    
    # 安全头
    add_header Strict-Transport-Security "max-age=31536000" always;
    add_header X-Frame-Options "SAMEORIGIN" always;
    add_header X-Content-Type-Options "nosniff" always;
    
    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
    
    access_log /www/wwwlogs/deepseek-c.log;
    error_log /www/wwwlogs/deepseek-c.error.log;
}
```

保存并重载 Nginx。

---

## ✅ 第五步：访问测试

1. 浏览器访问：`https://your-domain.com`
2. 应该能看到登录页面
3. 注册账号并测试聊天功能

---

## 🔄 常用维护命令

### 查看日志

```bash
# 应用日志
docker logs -f deepseek-c

# Nginx 访问日志
tail -f /www/wwwlogs/deepseek-c.log

# Nginx 错误日志
tail -f /www/wwwlogs/deepseek-c.error.log
```

### 重启服务

```bash
# 重启容器
docker restart deepseek-c

# 重启 Nginx（宝塔面板 → 软件商店 → Nginx → 重启）
# 或命令行
systemctl restart nginx
```

### 更新代码

```bash
cd /www/wwwroot/deepseek-c
git pull origin main
docker-compose down
docker-compose up -d --build
docker exec -it deepseek-c npx prisma migrate deploy
```

### 查看资源使用

```bash
# Docker 容器资源
docker stats deepseek-c

# 系统资源（宝塔面板首页可以看到）
htop
```

### 备份数据库

**宝塔面板 → 数据库 → PostgreSQL → deepseek-c → 备份**

或命令行：

```bash
# 备份
pg_dump -U deepseek_user deepseek-c > backup_$(date +%Y%m%d).sql

# 恢复
psql -U deepseek_user deepseek-c < backup_20250102.sql
```

---

## 🛠️ 故障排查

### 1. 容器无法启动

```bash
# 查看详细日志
docker logs deepseek-c

# 常见问题：
# - 端口 3000 被占用：改为其他端口
# - 环境变量错误：检查 .env.production
# - 数据库连接失败：检查 DATABASE_URL
```

### 2. 无法访问网站

```bash
# 检查容器是否运行
docker ps

# 检查端口是否监听
netstat -tlnp | grep 3000

# 检查防火墙
# 宝塔面板 → 安全 → 放行端口 3000（仅测试）

# 检查 Nginx 配置
nginx -t
```

### 3. 数据库连接失败

```bash
# 测试数据库连接
docker exec -it deepseek-c sh
node -e "const { PrismaClient } = require('@prisma/client'); new PrismaClient().\$connect().then(() => console.log('OK')).catch(e => console.error(e))"

# 检查 PostgreSQL 是否运行
# 宝塔面板 → 软件商店 → PostgreSQL → 状态
```

### 4. 构建失败

```bash
# 清理缓存重新构建
docker builder prune -af
docker build -t deepseek-c . --no-cache
```

---

## 🔐 安全建议

1. ✅ **修改宝塔默认端口**（8888 → 自定义）
2. ✅ **启用宝塔面板 SSL**
3. ✅ **定期更新系统** `yum update` 或 `apt update && apt upgrade`
4. ✅ **使用强密码**（数据库、宝塔面板）
5. ✅ **配置防火墙**只开放 80、443、SSH 端口
6. ✅ **定期备份数据库**
7. ✅ **不要泄露 `.env.production` 文件**
8. ✅ **重新生成 `NEXTAUTH_SECRET`** `openssl rand -base64 32`

---

## 📊 性能优化

### 1. Nginx 缓存

在 Nginx 配置中添加：

```nginx
# 静态资源缓存
location ~* \.(jpg|jpeg|png|gif|ico|css|js|svg)$ {
    proxy_pass http://127.0.0.1:3000;
    expires 7d;
    add_header Cache-Control "public, immutable";
}
```

### 2. 数据库优化

```bash
# 进入 PostgreSQL
sudo -u postgres psql deepseek-c

# 创建索引
CREATE INDEX idx_conversation_user ON "Conversation"("userId");
CREATE INDEX idx_message_conversation ON "Message"("conversationId");
```

### 3. Docker 资源限制

```yaml
# docker-compose.yml
services:
  deepseek-c:
    # ... 其他配置
    deploy:
      resources:
        limits:
          cpus: '1'
          memory: 1G
        reservations:
          memory: 512M
```

---

## 🎉 完成！

现在你的 DeepSeek-C 应用已经成功部署到生产环境了！

访问：`https://your-domain.com` 开始使用吧！

---

## 📞 获取帮助

- 查看日志：`docker logs -f deepseek-c`
- GitHub Issues: https://github.com/sujiu222/deepseek-c/issues
- 宝塔论坛：https://www.bt.cn/bbs/
